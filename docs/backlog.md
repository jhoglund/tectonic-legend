...
9. **✅ Stage gating** — difficulty picker is stage-aware; locked difficulties show the requirement to unlock them. Done 2026-05-15 (`b290627`). **Locking disabled 2026-05-15** (`STAGE_GATING_ENABLED = false` in `DifficultyPicker`) — every difficulty is playable; stage is a progress indicator, not a gate. Code is intact; flip the flag to re-enable.
