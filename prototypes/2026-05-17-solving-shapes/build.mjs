...
.proto-board{display:inline-grid;border-radius:var(--proto-rboard);
...
function renderBoard(rowsArr, sel, fill){
  ...
  for(r=0;r<rows;r++){for(c=0;c<cols;c++){
    ...
    // The outer frame is drawn by the edge cells themselves (row 0 /
    // col 0 / last row / last col), not a board border — so a selected
    // edge cell's ring can replace it cleanly.
    var topEdge=r===0?'cage':(grid[r-1][c]!==id?'cage':'inner');
    var leftEdge=c===0?'cage':(grid[r][c-1]!==id?'cage':'inner');
    var bottomEdge=r===rows-1?'cage':'none';
    var rightEdge=c===cols-1?'cage':'none';
    ...
    // Selection ring — a child layer, inset on the edges the cell draws
    // itself (incl. a board-edge cell's frame), outset on interior edges
    // a neighbour draws, so it replaces the line instead of doubling it.
    if(isSel){
      cell.classList.add('sel');
      var ring=document.createElement('span');
      ring.style.cssText='position:absolute;inset:0;pointer-events:none;box-shadow:'
